    ...
    private func ginit()
    {
        ...
        func complete(nu:NSURLResponse?,nd:NSData?,ne:NSError?)->Void
        {
            if(nd != nil)
            {
                ...
                let tempIMGView:NSImageView = NSImageView(frame: NSRect(x: 0, y: 0, width: tempImg.size.width/2.0, height: tempImg.size.height/2.0));
...
//        self.layer?.setNeedsDisplay();
        ...
        ci = tempImg.CGImageForProposedRect(&nr, context: nil, hints: nil)!;
        isInited = true;
